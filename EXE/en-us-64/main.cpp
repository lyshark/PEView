...
#define IMAGE_REL_BASED_DIR64       0x0A    // 64位地址重定位（扩展类型，必须保留）
...
DWORD64 GlobalFileSize = 0;          // 定义文件大小
DWORD64 GlobalFileBase = 0;          // 保存文件的基地址
DWORD64 IsOpen = 0;                  // 设置文件是否已经打开
...
namespace PEView
{
	...
	// 临时将RVA转换为FOA的函数
	// --------------------------------------------------
	DWORD64 RVAtoFOA(DWORD64 rva)
	{
		IsOpenFile();
		// auto SectionTables = IMAGE_FIRST_SECTION(NtHeader);    // 获取区段表
		auto SectionTables = IMAGE_FIRST_SECTION((PIMAGE_NT_HEADERS)NtHeader); // 强制转换适配IMAGE_FIRST_SECTION宏
	...
	// --------------------------------------------------
	DWORD64 FOAtoRVA(DWORD64 dwFOA)
	{
		IsOpenFile();
		DWORD64 NumberOfSectinsCount = 0;
	...
	// --------------------------------------------------
	DWORD64 HexStringToDec(char hexStr[])
	{
		DWORD64 i, m, n, temp = 0;
	...
	void GetProcessAddress(char* DllName, char* Function)
	{
		...
		if (hDll == NULL)
		{
			DWORD64 dwErr = GetLastError();
			printf("[-] 加载DLL失败！错误码:0x%016llX\n", dwErr);
		...
		// 成功获取地址，打印详细信息
		printf("[+] DLL加载基地址:0x%016llX\n", (DWORD64)hDll);
		printf("[+] 函数相对偏移（RVA）:0x%016llX\n", (DWORD64)pFunc - (DWORD64)hDll);
		printf("[+] 函数绝对地址（VA）:0x%016llX\n", (DWORD64)pFunc);

		// 释放DLL资源
		if (!FreeLibrary(hDll))
		{
			DWORD64 dwErr = GetLastError();
			printf("[-] 释放DLL资源失败！错误码:0x%016llX\n", dwErr);
		}
		DisplayLine(80);
	}
	...
	void OpenPeFile(LPCSTR FileName)
	{
		...
		LPVOID lpMapAddress = NULL; // 拆分定义，避免类型混淆
		...
		{
			// 设置读入文件基地址
			GlobalFileBase = (DWORD64)lpMapAddress;
			...
			// 判断是不是32位程序
			// 改为判断64位PE的Magic值（0x020B），原32位Magic值0x010B废弃
	...
	void ShowFileBasicInfo()
	{
		...
		printf("[PE头偏移]: 0x%016llX (从文件开始的偏移)\n", DosHeader->e_lfanew);

		// 3. NT头基本信息
		printf("[NT签名]: 0x%016llX (%s)\n", NtHeader->Signature,
		...
		printf("[时间戳]: 0x%016llX (", NtHeader->FileHeader.TimeDateStamp);
		...
		// 4. 可选头信息
		printf("[入口点RVA]: 0x%016llX (程序开始执行的相对虚拟地址)\n", NtHeader->OptionalHeader.AddressOfEntryPoint);
		printf("[镜像基址]: 0x%016llX (加载到内存中的首选基地址)\n", NtHeader->OptionalHeader.ImageBase);
		printf("[图像大小]: 0x%016llX 字节 (加载到内存后的总大小)\n", NtHeader->OptionalHeader.SizeOfImage);
		printf("[节区对齐]: 0x%016llX 字节 (内存中节区的对齐粒度)\n", NtHeader->OptionalHeader.SectionAlignment);
		printf("[文件对齐]: 0x%016llX 字节 (磁盘上节区的对齐粒度)\n", NtHeader->OptionalHeader.FileAlignment);
		...
		printf(")\n");
		printf("[栈大小]: 保留 0x%016llX 字节, 提交 0x%016llX 字节\n",
			NtHeader->OptionalHeader.SizeOfStackReserve, NtHeader->OptionalHeader.SizeOfStackCommit);
		printf("[堆大小]: 保留 0x%016llX 字节, 提交 0x%016llX 字节\n",
	...
	void ShowNtHead()
	{
		...
		printf("运行平台:             0x%016llX \t %08d \t %s \n",
			NtHeader->FileHeader.Machine,
			NtHeader->FileHeader.Machine,
			machineDesc);

		printf("区段数目:             0x%016llX \t %08d \t  PE文件包含的区段数量 \n",
			NtHeader->FileHeader.NumberOfSections,
			NtHeader->FileHeader.NumberOfSections);
		...
		printf("特征值:               0x%016llX \t %08d \t %s \n",
			NtHeader->FileHeader.Characteristics,
			NtHeader->FileHeader.Characteristics,
			characDesc.c_str());

		printf("可选头部大小:         0x%016llX \t %08d \t 可选头的字节大小 \n",
			NtHeader->FileHeader.SizeOfOptionalHeader,
			NtHeader->FileHeader.SizeOfOptionalHeader);

		printf("符号数量:             0x%016llX \t %08d \t 符号表中的符号数量 \n",
			NtHeader->FileHeader.NumberOfSymbols,
			NtHeader->FileHeader.NumberOfSymbols);

		printf("符号表指针:           0x%016llX \t %08d \t 符号表在文件中的偏移 \n",
		...
		printf("入口点(RVA):          0x%016llX \t %08d \t 入口点虚拟地址(VA: 0x%016llX) \n",
			NtHeader->OptionalHeader.AddressOfEntryPoint,
			NtHeader->OptionalHeader.AddressOfEntryPoint,
			entryVA);

		printf("镜像基址:             0x%016llX \t %08d \t 加载到内存的首选基地址 \n",
			NtHeader->OptionalHeader.ImageBase,
			NtHeader->OptionalHeader.ImageBase);

		printf("镜像大小:             0x%016llX \t %08d \t 内存中整个镜像的大小(字节) \n",
			NtHeader->OptionalHeader.SizeOfImage,
			NtHeader->OptionalHeader.SizeOfImage);

		printf("代码基址(RVA):        0x%016llX \t %08d \t 代码段的起始相对虚拟地址 \n",
			NtHeader->OptionalHeader.BaseOfCode,
			NtHeader->OptionalHeader.BaseOfCode);

		/*
		printf("数据基址(RVA):        0x%016llX \t %08d \t 数据段的起始相对虚拟地址 \n",  // 原代码遗漏
			NtHeader->OptionalHeader.BaseOfData,
			NtHeader->OptionalHeader.BaseOfData);
			*/

		printf("代码大小:             0x%016llX \t %08d \t 代码段的总大小(字节) \n",  // 原代码遗漏
			NtHeader->OptionalHeader.SizeOfCode,
			NtHeader->OptionalHeader.SizeOfCode);

		printf("已初始化数据大小:     0x%016llX \t %08d \t 已初始化数据段的大小(字节) \n",  // 原代码遗漏
			NtHeader->OptionalHeader.SizeOfInitializedData,
			NtHeader->OptionalHeader.SizeOfInitializedData);

		printf("未初始化数据大小:     0x%016llX \t %08d \t 未初始化数据段的大小(字节) \n",  // 原代码遗漏
			NtHeader->OptionalHeader.SizeOfUninitializedData,
			NtHeader->OptionalHeader.SizeOfUninitializedData);

		printf("内存对齐:             0x%016llX \t %08d \t 内存中区块的对齐粒度(字节) \n",
			NtHeader->OptionalHeader.SectionAlignment,
			NtHeader->OptionalHeader.SectionAlignment);

		printf("文件对齐:             0x%016llX \t %08d \t 文件中区块的对齐粒度(字节) \n",
			NtHeader->OptionalHeader.FileAlignment,
			NtHeader->OptionalHeader.FileAlignment);
		...
		printf("子系统:               0x%016llX \t %08d \t %s \n",
			NtHeader->OptionalHeader.Subsystem,
			NtHeader->OptionalHeader.Subsystem,
			subsystemDesc);

		printf("首部大小:             0x%016llX \t %08d \t DOS头+NT头+区段表的总大小 \n",
			NtHeader->OptionalHeader.SizeOfHeaders,
			NtHeader->OptionalHeader.SizeOfHeaders);

		printf("校验和:               0x%016llX \t %08d \t 用于验证文件完整性(通常为0) \n",
			NtHeader->OptionalHeader.CheckSum,
			NtHeader->OptionalHeader.CheckSum);

		printf("数据目录数量:         0x%016llX \t %08d \t 数据目录项的数量(通常为16) \n",  // 原代码描述优化
			NtHeader->OptionalHeader.NumberOfRvaAndSizes,
			NtHeader->OptionalHeader.NumberOfRvaAndSizes);

		// 链接器版本
		printf("链接器主版本:         0x%016llX \t %08d \n",  // 原代码遗漏
			NtHeader->OptionalHeader.MajorLinkerVersion,
			NtHeader->OptionalHeader.MajorLinkerVersion);
		printf("链接器次版本:         0x%016llX \t %08d \n",  // 原代码遗漏
		...
		printf("Win32版本值:          0x%016llX \t %08d \t 通常为0(保留) \n",
		...
		printf("DLL特征:              0x%016llX \t %08d \t %s \n",
			NtHeader->OptionalHeader.DllCharacteristics,
			NtHeader->OptionalHeader.DllCharacteristics,
			dllCharacDesc.c_str());

		// 栈和堆相关（补充说明）
		printf("栈保留大小:           0x%016llX \t %08d \t 进程栈的保留大小 \n",
			NtHeader->OptionalHeader.SizeOfStackReserve,
			NtHeader->OptionalHeader.SizeOfStackReserve);
		printf("栈提交大小:           0x%016llX \t %08d \t 进程栈的初始提交大小 \n",
			NtHeader->OptionalHeader.SizeOfStackCommit,
			NtHeader->OptionalHeader.SizeOfStackCommit);
		printf("堆保留大小:           0x%016llX \t %08d \t 进程堆的保留大小 \n",
			NtHeader->OptionalHeader.SizeOfHeapReserve,
			NtHeader->OptionalHeader.SizeOfHeapReserve);
		printf("堆提交大小:           0x%016llX \t %08d \t 进程堆的初始提交大小 \n",
			NtHeader->OptionalHeader.SizeOfHeapCommit,
			NtHeader->OptionalHeader.SizeOfHeapCommit);

		printf("加载器标志:           0x%016llX \t %08d \t 已废弃(通常为0) \n",  // 补充说明
	...
	void ShowOptionalDataDirectoryInfo()
	{
		...
		DWORD64 ImageBase = NtHeader->OptionalHeader.ImageBase; // 基地址，用于计算VA
		...
		for (int x = 0; x < Data_Size; x++)
		{
			...
			DWORD64 rva = dir.VirtualAddress;
			DWORD64 size = dir.Size;
			DWORD64 foa = RVAtoFOA(rva);
			DWORD64 va = (rva != 0) ? (ImageBase + rva) : 0; // 计算虚拟地址(VA)
			bool is_valid = (rva != 0 && size != 0);       // 有效性判断（RVA和Size均非0）

			// 打印基础信息
			printf("%03d \t %-16s 0x%016llX \t 0x%016llX \t 0x%016llX \t %010d \t 0x%016llX \t\t %s \t ",
				x + 1,
	...
	void ShowImportByDll()
	{
		...
		while (ImportTable->Name != 0) // 遍历所有导入描述符（以全0项结束）
		{
			...
			DWORD64 nameRva = ImportTable->Name;
			DWORD64 nameFoa = RVAtoFOA(nameRva);
			...
			// 打印完整信息（控制列宽确保对齐）
			printf("%-6d %-20s 0x%016llX \t 0x%016llX \t 0x%016llX \t 0x%016llX \t 0x%016llX \t 0x%016llX \t 0x%016llX \t\t %-19s %-16s 0x%016llX \t 0x%016llX \n",
				Count,
				dllName,               // DLL名称
				intRva,                // INT的RVA
				intFoa,                // INT的FOA
	...
	void ShowImportByName(char* Dll)
	{
		...
		while (ImportTable->Name != 0)
		{
				...
				&& Iat->u1.Ordinal != 0)
			{
				...
				// 打印信息
				printf("%-6d %-10s %-8hd 0x%016llX \t 0x%016llX \t 0x%016llX \t 0x%016llX \t 0x%016llX \t 0x%016llX \t %-20s \n",
					funcCount++,
					importType,
					hint,
					currentIntRva,
	...
	{
		IsOpenFile();
		DWORD64 imageBase = NtHeader->OptionalHeader.ImageBase; // 模块基地址，用于计算VA
		DWORD64 importDirRva = NtHeader->OptionalHeader.DataDirectory[1].VirtualAddress;
		auto ImportTable = (PIMAGE_IMPORT_DESCRIPTOR)(RVAtoFOA(importDirRva) + GlobalFileBase);
	...
	void ShowImportAll()
	{
		...
		DisplayLine(200);
		printf("导入表全局信息\n");
		printf("导入表数据目录RVA: 0x%016llX, FOA: 0x%016llX\n", importDirRva, RVAtoFOA(importDirRva));
		printf("模块加载基地址: 0x%016llX\n", imageBase);
		DisplayLine(200);
		...
		while (ImportTable->Name != 0)
		{
			...
			DWORD64 intRva = ImportTable->OriginalFirstThunk;  // INT（导入名称表）RVA
			DWORD64 iatRva = ImportTable->FirstThunk;          // IAT（导入地址表）RVA
			DWORD64 descRva = (DWORD64)((BYTE*)ImportTable - (BYTE*)GlobalFileBase) + FOAtoRVA((DWORD64)((BYTE*)ImportTable - (BYTE*)GlobalFileBase) - GlobalFileBase); // 导入描述符自身RVA
			DWORD64 descFoa = RVAtoFOA(descRva);               // 导入描述符自身FOA
			...
			printf("[*] 导入模块 [%d]:%s\n", dllIndex++, dllName);
			printf("  导入描述符信息:\n");
			printf("  - 描述符RVA: 0x%016llX, FOA: 0x%016llX\n", descRva, descFoa);
			printf("  - TimeDateStamp: 0x%016llX（%s）\n",
				ImportTable->TimeDateStamp,
				ImportTable->TimeDateStamp == 0 ? "未绑定" : "已绑定");
			printf("  - ForwarderChain: 0x%016llX（%s）\n",
				ImportTable->ForwarderChain,
				ImportTable->ForwarderChain == (DWORD64)-1 ? "无转发" : "存在转发");
			printf("  - INT（导入名称表）RVA: 0x%016llX, FOA: 0x%016llX\n", intRva, RVAtoFOA(intRva));
			printf("  - IAT（导入地址表）RVA: 0x%016llX, FOA: 0x%016llX\n", iatRva, RVAtoFOA(iatRva));
			DisplayLine(200);
				...
				(Iat == nullptr || Iat->u1.Ordinal != 0))
			{
				...
				WORD hint = 0;
				const char* funcInfo = "未知";
				DWORD64 intRawData = 0;  // INT表项原始数据
				DWORD64 iatRawData = 0;  // IAT表项原始数据
				DWORD64 intVa = 0;       // INT表项内存地址
				DWORD64 iatVa = 0;       // IAT表项内存地址
				...
				else if (ImportTable->ForwarderChain != (DWORD64)-1)
				{
					status = "存在函数转发";
				}

				// 输出当前函数详细信息
				printf("[%6d] \t %-10s %-8hd 0x%016llX \t 0x%016llX \t 0x%016llX \t 0x%016llX \t %-20s \t %s \n",
					funcIndex++,
					importType,
					hint,
	...
	void ShowExport()
	{
		IsOpenFile();
		// 0. 获取镜像基地址
		DWORD64 ImageBase = NtHeader->OptionalHeader.ImageBase;
		...
		DWORD64 FunctionCount = ExportTable->NumberOfFunctions;
		DWORD64 Base = ExportTable->Base;  // 起始序号
		...
		DWORD64* Addr_Table = (DWORD64*)(RVAtoFOA(ExportTable->AddressOfFunctions) + GlobalFileBase);
		DWORD64* Name_Table = (DWORD64*)(RVAtoFOA(ExportTable->AddressOfNames) + GlobalFileBase);
		WORD* Id_Table = (WORD*)(RVAtoFOA(ExportTable->AddressOfNameOrdinals) + GlobalFileBase);

		// ---------------------- 输出导出表全局信息 ----------------------
		DisplayLine(120);
		printf("导出表全局信息\n");
		printf("导出表数据目录RVA: 0x%016llX, FOA: 0x%016llX\n", exportDirRva, exportDirFoa);
		printf("特征值(Characteristics): 0x%016llX\n", ExportTable->Characteristics);
		printf("时间戳(TimeDateStamp): 0x%016llX -> %04d-%02d-%02d %02d:%02d:%02d\n",
		...
		// 6. 遍历所有导出函数（地址表）
		for (DWORD64 i = 0; i < FunctionCount; ++i)
		{
			DWORD64 funcRva = Addr_Table[i];
			...
			// 6.4 输出当前函数信息
			printf("%8d \t 0x%016llX \t 0x%016llX \t 0x%016llX \t %-8s \t %-20s \t %-6s \t %s\n",
				i + Base,         // 导出序号（Base + 索引）
				funcRva,          // 函数RVA
				funcVa,           // 函数VA（内存地址）
				funcFoa,          // 函数FOA（文件偏移）
	...
	void ShowFixRelocPage()
	{
		...
		// 1. 获取基础信息
		DWORD64 base = NtHeader->OptionalHeader.ImageBase;                   // 原始映像基地址
		PIMAGE_DATA_DIRECTORY relocDir = &NtHeader->OptionalHeader.DataDirectory[5];  // 重定位表数据目录
		DWORD64 RelocRVA = relocDir->VirtualAddress;                         // 重定位表RVA
		...
		DWORD64 newBase = base + 0x100000;  // 示例:原基地址+0x10000

		// ---------------------- 输出重定位表全局信息 ----------------------
		DisplayLine(120);
		printf("重定位表全局信息\n");
		printf("原始映像基地址: 0x%016llX\n", base);
		printf("模拟新基地址(示例): 0x%016llX\n", newBase);
		printf("重定位表数据目录RVA: 0x%016llX, FOA: 0x%016llX\n", RelocRVA, RelocFOA);
		printf("重定位表总大小: 0x%016llX 字节\n", RelocSize);
		DisplayLine(120);
		...
		while (Reloc->SizeOfBlock != 0)
		{
			...
			DWORD64 blockVA = base + Reloc->VirtualAddress;

			// 计算重定位项个数（块大小减去块头大小，再除以每个项的大小2字节）
			DWORD64 entryCount = (Reloc->SizeOfBlock - sizeof(IMAGE_BASE_RELOCATION)) / 2;

			// 输出当前块的基本信息
			printf("%d \t 0x%016llX \t 0x%016llX \t 0x%016llX \t 0x%04X \t %d \n",
				blockIndex,
			...
			if (entryCount > 0)
			{
				...
				for (DWORD64 i = 0; i < entryCount; i++)
				{
					...
					// 计算完整RVA（块起始RVA + 项偏移）
					DWORD64 entryRVA = Reloc->VirtualAddress + offset;

					// 计算项的FOA和文件中地址
					DWORD64 entryFOA = RVAtoFOA(entryRVA);
					DWORD64* entryAddrInFile = (DWORD64*)(GlobalFileBase + entryFOA);  // 文件中存储的原始地址

					// 计算重定位后地址:原始地址 - 原基地址 + 新基地址
					DWORD64 relocatedAddr = (*entryAddrInFile) - base + newBase;
					...
					case IMAGE_REL_BASED_DIR64:       typeDesc = "DIR64（64位完整地址）"; break; // 新增64位重定位类型描述
						// 可根据需要补充其他类型（如IMAGE_REL_BASED_DIR64等）
					}

					// 输出当前重定位项信息
					printf("  ├─ %d \t %d \t %-15s \t 0x%04X \t 0x%016llX \t 0x%016llX \t 0x%016llX \n",
						i + 1,
						type,
						typeDesc,
						offset,
			...
			// 移动到下一个重定位块
			Reloc = (PIMAGE_BASE_RELOCATION)((DWORD64)Reloc + Reloc->SizeOfBlock);
		}
	...
	{
		IsOpenFile();
		DWORD64 oldBase = NtHeader->OptionalHeader.ImageBase;  // 原始映像基地址
		DWORD64 newBase = oldBase + 0x100000;                   // 模拟新基地址（示例:原基地址+0x10000）
		...
		printf("原始映像基地址: 0x%016llX\n", oldBase);
		printf("模拟新基地址(示例): 0x%016llX\n", newBase);
		printf("重定位表RVA: 0x%016llX, FOA: 0x%016llX, 总大小: 0x%016llX 字节\n", RelocRVA, RelocFOA, RelocSize);
		DisplayLine(120);
		...
		while (Reloc->SizeOfBlock != 0)
		{
			...
			// 计算当前块的基础信息
			DWORD64 blockFOA = (DWORD64)Reloc - GlobalFileBase;  // 块在文件中的偏移
			DWORD64 blockVA = oldBase + Reloc->VirtualAddress;  // 块的内存起始VA
			DWORD64 entryCount = (Reloc->SizeOfBlock - sizeof(IMAGE_BASE_RELOCATION)) / 2;  // 项数量
			...
			{
				// 解析重定位项类型和偏移
				DWORD64 type = Offset[i].Type;
				DWORD64 offset = Offset[i].Offset;

				// 计算项的完整RVA、FOA和文件中地址
				DWORD64 entryRVA = Reloc->VirtualAddress + offset;
				DWORD64 entryFOA = RVAtoFOA(entryRVA);
				DWORD64* entryFileAddr = (DWORD64*)(GlobalFileBase + entryFOA);  // 文件中存储的原始地址

				// 计算重定位相关地址
				DWORD64 originalVA = *entryFileAddr;                // 原始VA（文件中存储的地址）
				DWORD64 relocatedVA = originalVA - oldBase + newBase;  // 重定位后VA（基于新基地址）
				...
				case IMAGE_REL_BASED_DIR64:       typeDesc = "DIR64（64位完整地址）"; break;
				}
				...
				{
					//DWORD64 targetRVA = HexStringToDec(GetRva);
					DWORD64 targetRVA = strtoull(GetRva, nullptr, 16);  // 适配64 原HexStringToDec→strtoull（16进制转64位）
				...
				{
					printf(
						"%-6d | 0x%016llX | 0x%016llX | 0x%016llX | 0x%04X | %-6d | %-6d | %-4d | %-14s | 0x%04X | 0x%016llX | 0x%016llX | 0x%016llX | 0x%016llX \n",
		...
		)
	{
		...
		// 计算当前目录的RVA和FOA
		DWORD64 dirFOA = (DWORD64)pResDir - GlobalFileBase;  // 目录在文件中的偏移
		DWORD64 dirRVA = FOAtoRVA(dirFOA);                 // 目录的RVA

		// 输出当前目录的基本信息（按级别区分）
		const char* levelName[] = { "", "类型目录", "名称/ID目录", "语言目录" };
		printf("\n%s[ %s ] 信息:\n", parentInfo, levelName[level]);
		printf("%s- 目录RVA: 0x%016llX, FOA: 0x%016llX\n", parentInfo, dirRVA, dirFOA);
		...
		DWORD64 entryCount = pResDir->NumberOfNamedEntries + pResDir->NumberOfIdEntries;
		...
		for (DWORD64 i = 0; i < entryCount; ++i)
		{
			...
			else
			{
				...
				printf("%s- 条目类型: 资源数据\n", childParentInfo);
				printf("%s- 数据RVA: 0x%016llX, FOA: 0x%016llX\n", childParentInfo, dataRVA, dataFOA);
				printf("%s- 数据大小: %d 字节 (0x%016llX)\n", childParentInfo, dataSize, dataSize);
	...
	void ShowResource()
	{
		...
		DisplayLine(120);
		printf("资源表全局信息\n");
		printf("资源表数据目录RVA: 0x%016llX, FOA: 0x%016llX\n", resRVA, resFOA);
		printf("资源表总大小: %d 字节 (0x%016llX)\n", resSize, resSize);
		printf("资源表在文件中的基地址: 0x%016llX (GlobalFileBase + FOA)\n", (DWORD64)pRootResDir);
		DisplayLine(120);
	...
	void VA_To_FOA(DWORD64 dwVA)
	{
		...
		{
			DisplayLine(80);
			printf("[-] 无效的VA地址:0x%016llX（小于基地址0x%016llX）\n", dwVA, dwImageBase);
			DisplayLine(80);
			return;
		}

		DWORD64 NumberOfSectionsCount = NtHeader->FileHeader.NumberOfSections; // 修正变量名拼写错误
		bool bFound = false; // 标记是否找到对应节区

		DisplayLine(80);
		printf("[*] VA转FOA转换开始:\n");
		printf("[*] 目标VA地址:0x%016llX\n", dwVA);
		printf("[*] 模块基地址（ImageBase）:0x%016llX\n", dwImageBase);
		...
		for (DWORD64 i = 0; i < NumberOfSectionsCount; i++)
		{
			...
			// 计算当前节区的虚拟地址范围（内存中）
			DWORD64 dwSectionVAStart = dwImageBase + pCurSection->VirtualAddress; // 节区VA起始
			// 节区VA结束:使用VirtualSize（内存中实际大小），左闭右开
			DWORD64 dwSectionVAEnd = dwSectionVAStart + pCurSection->Misc.VirtualSize;

			// 打印当前节区信息（辅助定位）
			printf("[节区 %d] 名称:%s | VA范围:0x%016llX - 0x%016llX | 节区RVA起始:0x%016llX | 内存大小:0x%016llX\n",
				i + 1,
				pCurSection->Name,
				dwSectionVAStart,
			...
			if (dwVA >= dwSectionVAStart && dwVA < dwSectionVAEnd)
			{
				...
				// 计算转换结果
				DWORD64 dwRVA = dwVA - dwImageBase; // RVA = VA - 基地址
				DWORD64 dwFOA = pCurSection->PointerToRawData + (dwRVA - pCurSection->VirtualAddress); // FOA = 节区文件偏移 + (RVA - 节区RVA起始)

				// 输出详细转换信息
				DisplayLine(80);
				printf("[+] 找到VA所属节区:\n");
				printf("    节区名称:%s\n", pCurSection->Name);
				printf("    节区VA范围:0x%016llX - 0x%016llX（有效范围）\n", dwSectionVAStart, dwSectionVAEnd - 1);
				printf("    节区RVA起始:0x%016llX | 节区文件偏移:0x%016llX | 内存大小:0x%016llX\n",
					pCurSection->VirtualAddress,
				...
				printf("    转换过程:\n");
				printf("    RVA = VA - 基地址\n");
				printf("    RVA = 0x%016llX - 0x%016llX = 0x%016llX\n", dwVA, dwImageBase, dwRVA);
				printf("    FOA = 节区文件偏移 + (RVA - 节区RVA起始)\n");
				printf("    FOA = 0x%016llX + (0x%016llX - 0x%016llX) = 0x%016llX\n",
					pCurSection->PointerToRawData,
					dwRVA,
					pCurSection->VirtualAddress,
					dwFOA);
				DisplayLine(60);
				printf("    最终结果:\n");
				printf("    VA:0x%016llX --> RVA:0x%016llX --> FOA:0x%016llX\n", dwVA, dwRVA, dwFOA);
				DisplayLine(80);
				break; // 找到后退出循环
			}
		}
		...
		{
			DisplayLine(80);
			printf("[-] 转换失败:VA 0x%016llX 不在任何节区的虚拟地址范围内\n", dwVA);
			printf("[-] 可能原因:VA属于未分配的内存区域或超出模块镜像大小（0x%016llX）\n", NtHeader->OptionalHeader.SizeOfImage);
			DisplayLine(80);
		}
	}
	...
	void RVA_To_FOA(DWORD64 dwRVA)
	{
		...
		for (DWORD64 i = 0; i < dwSectionCount; i++)
		{
			...
			// 打印当前节区信息（辅助调试）
			printf("[节区 %d] 名称:%s | RVA范围:0x%016llX - 0x%016llX | 节区文件偏移:0x%016llX\n",
				i + 1,
				pCurSection->Name,
				dwSectionRVAStart,
				dwSectionRVAEnd,
			...
			if (dwRVA >= dwSectionRVAStart && dwRVA <= dwSectionRVAEnd)
			{
				...
				// 计算转换结果
				DWORD64 dwVA = dwImageBase + dwRVA; // 虚拟地址（VA）
				DWORD64 dwFOA = pCurSection->PointerToRawData + (dwRVA - dwSectionRVAStart); // 文件偏移（FOA）

				// 输出详细转换信息
				DisplayLine(80);
				printf("[+] 找到RVA所属节区:\n");
				printf("    节区名称:%s\n", pCurSection->Name);
				printf("    节区RVA起始:0x%016llX | 节区RVA结束:0x%016llX\n", dwSectionRVAStart, dwSectionRVAEnd);
				printf("    节区文件偏移:0x%016llX | 节区虚拟大小:0x%016llX | 节区文件大小:0x%016llX\n",
					pCurSection->PointerToRawData,
					pCurSection->Misc.VirtualSize,
				...
				DisplayLine(60);
				printf("    最终结果:\n");
				printf("    RVA:0x%016llX --> VA:0x%016llX --> FOA:0x%016llX\n", dwRVA, dwVA, dwFOA);
				DisplayLine(80);
				break; // 找到后退出循环
			}
		}
		...
		{
			DisplayLine(80);
			printf("[-] 转换失败:RVA 0x%016llX 不在任何节区范围内\n", dwRVA);
	...
	void FOA_To_VA(DWORD64 dwFOA)
	{
		...
		{
			DisplayLine(80);
			printf("[-] 无效的FOA地址:0x%016llX（超过文件总大小0x%016llX）\n", dwFOA, GlobalFileSize);
			DisplayLine(80);
			return;
		}
		...
		for (DWORD64 i = 0; i < NumberOfSectionsCount; i++)
		{
			...
			DWORD64 dwSectionFOAEnd = dwSectionFOAStart + pCurSection->SizeOfRawData; // 结束位置为开区间

			// 打印当前节区信息（辅助定位）
			printf("[节区 %d] 名称:%s | 文件偏移范围:0x%016llX - 0x%016llX | 节区虚拟地址:0x%016llX\n",
				i + 1,
				pCurSection->Name,
				dwSectionFOAStart,
			...
			if (dwFOA >= dwSectionFOAStart && dwFOA < dwSectionFOAEnd)
			{
				...
				// 计算转换结果
				DWORD64 dwRVA = pCurSection->VirtualAddress + (dwFOA - dwSectionFOAStart); // RVA = 节区虚拟地址 + (FOA - 节区文件偏移)
				DWORD64 dwVA = dwImageBase + dwRVA; // VA = 基地址 + RVA

				// 输出详细转换信息
				DisplayLine(80);
				printf("[+] 找到FOA所属节区:\n");
				printf("    节区名称:%s\n", pCurSection->Name);
				printf("    节区文件偏移范围:0x%016llX - 0x%016llX（有效范围）\n", dwSectionFOAStart, dwSectionFOAEnd - 1);
				printf("    节区虚拟地址（RVA起始）:0x%016llX | 节区文件大小:0x%016llX\n",
					pCurSection->VirtualAddress,
					pCurSection->SizeOfRawData);
				DisplayLine(60);
				printf("    转换过程:\n");
				printf("    RVA = 节区虚拟地址 + (FOA - 节区文件偏移起始)\n");
				printf("    RVA = 0x%016llX + (0x%016llX - 0x%016llX) = 0x%016llX\n",
					pCurSection->VirtualAddress,
					dwFOA,
					dwSectionFOAStart,
					dwRVA);
				printf("    VA = 基地址 + RVA\n");
				printf("    VA = 0x%016llX + 0x%016llX = 0x%016llX\n",
					dwImageBase,
					dwRVA,
					dwVA);
				DisplayLine(60);
				printf("    最终结果:\n");
				printf("    FOA:0x%016llX --> RVA:0x%016llX --> VA:0x%016llX\n", dwFOA, dwRVA, dwVA);
				DisplayLine(80);
				break; // 找到后退出循环
			}
		}
		...
		{
			DisplayLine(80);
			printf("[-] 转换失败:FOA 0x%016llX 不在任何节区的文件偏移范围内\n", dwFOA);
	...
	// 增强版:完善校验与转换过程可视化
	// --------------------------------------------------
	void VA_To_RVA(DWORD64 dwVA)
	{
		IsOpenFile(); // 确保文件已打开

		DWORD64 dwImageBase = NtHeader->OptionalHeader.ImageBase;
		DWORD64 dwSizeOfImage = NtHeader->OptionalHeader.SizeOfImage;
		...
		if (dwRVA >= dwSizeOfImage)
		{
			...
			printf("    VA:0x%016llX | 基地址:0x%016llX | 计算得到RVA:0x%016llX\n", dwVA, dwImageBase, dwRVA);
			printf("    模块总大小（SizeOfImage）:0x%016llX，RVA超出有效范围\n", dwSizeOfImage);
			DisplayLine(80);
			return;
		}
		...
		printf("[*] 目标VA地址:0x%016llX\n", dwVA);
		printf("[*] 模块基地址（ImageBase）:0x%016llX\n", dwImageBase);
		printf("[*] 模块总大小（SizeOfImage）:0x%016llX\n", dwSizeOfImage);
		DisplayLine(60);
		printf("    转换过程:\n");
		printf("    RVA = VA - 基地址\n");
		printf("    RVA = 0x%016llX - 0x%016llX = 0x%016llX\n", dwVA, dwImageBase, dwRVA);
		DisplayLine(60);
		printf("    最终结果:\n");
		printf("    VA:0x%016llX --> RVA:0x%016llX（有效，在模块范围内）\n", dwVA, dwRVA);
		DisplayLine(80);
	}
	...
	void RVA_To_VA(DWORD64 dwRVA)
	{
		...
		{
			DisplayLine(80);
			printf("[-] 无效的RVA地址:0x%016llX（超过模块总大小0x%016llX）\n", dwRVA, dwSizeOfImage);
			DisplayLine(80);
			return;
		}
		...
		printf("    最终结果:\n");
		printf("    RVA:0x%016llX --> VA:0x%016llX\n", dwRVA, dwVA);
		DisplayLine(80);
	}
	...
	void SearchString(long long StartAddr, long long SearchLen, const std::string& target_str)
	{
		...
		if (match_offsets.empty())
		{
			...
			for (long long offset : match_offsets)
			{
				...
				{ // 确保FOA在32位范围内
					DWORD64 rva = FOAtoRVA((DWORD64)offset);
	...
	void ModuleStatus()
	{
		...
		DWORD64 secDirSize = NtHeader->OptionalHeader.DataDirectory[4].Size;  // 安全目录（证书）大小
		DWORD64 dbgDirSize = NtHeader->OptionalHeader.DataDirectory[6].Size;  // 调试目录大小
...
namespace PEDisassemble
{
	...
	// 在文件偏移位置读入数据
	BOOL ReadPEFileOffset(IN LPSTR file_path, IN DWORD64 Offset, IN DWORD64 Size, OUT LPVOID* pFileBuffer)
	{
		FILE* fp = NULL;
