...
public:
	...
	static ResponseData handle_sub_calculator(const std::vector<std::string>& params)
	{
		...
		try
		{
			...
			if (!resultObj) throw std::bad_alloc();
			// 补充减法无符号提示（避免用户误解负数）
			cJSON_AddStringToObject(resultObj, "note", "Subtraction is based on DWORD unsigned calculation; negative numbers are automatically converted to modulo 2^32 values");
			cJSON_AddItemToObject(root, "calculation_result", resultObj);
